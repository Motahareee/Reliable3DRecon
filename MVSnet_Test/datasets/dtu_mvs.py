...
class MVSDataset(Dataset):
    def __init__(self, datapath, listfile, mode, nviews, cams, ndepths=192, interval_scale=1.06, **kwargs):
    ...
    def build_list(self):
        ...
        for scan in scans:
            ...
            cam_id = np.where(np.array(self.cams) == 0)[0]
            ...
            with open(os.path.join(self.datapath, pair_file)) as f:
                ...
                for view_idx in range(num_viewpoint):
                    ...
                    for x in f.readline().rstrip().split()[1::2]:
                        if int(x) not in cam_id:
                            src_views.append(int(x))

                    if self.cams[view_idx] == 1:
                        metas.append((scan, ref_view, src_views))
    ...
    def read_img(self, filename):
        ...
        # Crop to (894, 1600)
        np_img = np_img[:-4, :] # specific for our dataset, must be the size can be divided by 16
